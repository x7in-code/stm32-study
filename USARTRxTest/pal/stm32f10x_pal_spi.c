...
void PAL_SPI_Init(PalSPI_HandleTypeDef *Handle)
{
	...
	GPIOMode_TypeDef txPinMode = Handle->Init.Advanced.SPITxPinMode == SPI_TX_PIN_MODE_PP ? GPIO_Mode_AF_PP : GPIO_Mode_AF_OD;
	...
	if(Handle->Init.SPIx == SPI1)
	{
		if(Handle->Init.Advanced.Remap == 0) // NSS, SCK, MISO, MOSI PA4, PA5, PA6, PA7
		{
			...
			GPIOInitStruct.GPIO_Mode = txPinMode;
			GPIOInitStruct.GPIO_Speed = GPIO_Speed_50MHz;
			GPIO_Init(GPIOA, &GPIOInitStruct);
		}
...
static void DisableTxPin(PalSPI_HandleTypeDef *Handle);
static void ResumeTxPin(PalSPI_HandleTypeDef *Handle);
...
static void ResumeTxPin(PalSPI_HandleTypeDef *Handle)
{
	...
	if(Handle->Init.SPIx == SPI1)
	{
		...
		{
			GPIO_InitStruct.GPIO_Pin = GPIO_Pin_7;
			GPIO_InitStruct.GPIO_Mode = txPinMode;
